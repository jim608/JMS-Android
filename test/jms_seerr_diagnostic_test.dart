...
void main() {
  ...
  test('Seerr 403 on auth/me means missing or expired session, not permission', () {
    ...
    expect(assess('/api/v1/issue', 403, authError, cookie: true, verified: true).failureCode,
        'permission_denied');
  });
  ...
  test('unknown JSON rejection, HTML, challenge and redirect remain distinct', () {
    ...
    expect(assess('/api/v1/status', 403, '<html>Denied</html>', contentType: 'text/html').failureCode,
        'unexpected_html');
    expect(assess('/api/v1/status', 200, '<html>Sign in</html>', contentType: 'text/html').failureCode,
        'unexpected_html');
    expect(assess('/api/v1/status', 403, '<html>Challenge</html>', contentType: 'text/html',
  ...
  test('Cloudflare Error 1000 is an edge DNS failure, not a Seerr permission denial', () {
    final json = assess('/api/v1/status', 403,
        {'status': 403, 'cloudflare_error': true, 'error_code': 1000,
          'error_name': 'dns_loop', 'ray_id': 'a3f503042d217d73'});
  ...
  test('session cookie parses multiple Set-Cookie without splitting Expires', () {
    final header = 'csrf=ignored; Expires=Wed, 21 Oct 2037 07:28:00 GMT; Path=/, '
        'connect.sid=TEST_ONLY; Path=/; Domain=legacy-seerr.example.invalid; Secure; HttpOnly';
    expect(seerrSessionCookieFromHeader(header, 'legacy-seerr.example.invalid'), 'connect.sid=TEST_ONLY');
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Max-Age=0; Path=/', 'legacy-seerr.example.invalid'), isNull);
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Domain=other.invalid', 'legacy-seerr.example.invalid'), isNull);
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Path=/other', 'legacy-seerr.example.invalid'), isNull);
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Path=/api/v1/auth', 'legacy-seerr.example.invalid'), isNull);
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Secure', 'legacy-seerr.example.invalid',
        secureConnection: false), isNull);
    expect(seerrSessionCookieFromHeader('connect.sid=TEST_ONLY; Expires=Wed, 21 Oct 2015 07:28:00 GMT',
        'legacy-seerr.example.invalid'), isNull);
  });
}
