...
AccountModel seerrFixtureAccount({String id = 'aabbcc', bool bound = true}) =>
    AccountModel(
      name: '測試使用者',
      id: id,
      avatar: '',
      ...
      credentials: CredentialsModel.internal(
          serverId: 'fixture-server', url: 'https://example.invalid/jellyfin'),
      seerrCredentials: SeerrCredentialsModel(
          serverUrl: jmsSeerrSource,
          linkedServerId: bound ? 'fixture-server' : ''),
    );
...
class SeerrFixtureStore extends SeerrSessionStore {
  ...
  Future<String?> readForRequest(AccountModel account, Uri requestUri) async {
    ...
    final restored = SeerrCookieJar(requestUri);
    restored.ingest(requestUri, [
      for (final pair in cookie.split(';')) '${pair.trim()}; Path=/; Secure'
    ]);
    jars[scope] = restored;
  ...
  Future<bool> writeFromResponse(AccountModel account, Uri responseUri,
    ...
    final jar = jars.putIfAbsent(scope, () => SeerrCookieJar(responseUri));
    final received = jar.ingest(responseUri, setCookieHeaders);
    final cookie = jar.headerFor(responseUri.replace(path: '/api/v1/auth/me'));
    if (cookie == null) {
      values.remove(scope);
    } else {
      values[scope] = cookie;
...
class SeerrFixture {
  ...
  Map<String, dynamic> get user => {
        ...
        'permissions': 4194304 | 32
      };
  Map<String, dynamic> get media =>
      {'id': 7, 'tmdbId': 42, 'mediaType': 'movie', 'status': 3};
  ...
  Future<http.Response> handle(http.Request call) async {
    ...
    if (path == '/api/v1/auth/me') {
      final status = nextMeStatus ?? authStatus;
      nextMeStatus = null;
      return json(
          status == 403
              ? {
    ...
    if (path.endsWith('/quickconnect/authenticate')) {
      return json(user, cookie: true);
    }
    ...
    if (path.endsWith('/watchproviders/regions') || path.endsWith('/regions')) {
      return json([]);
    }
    if (path.contains('/ratings')) return json({});
    if (path.contains('/recommendations') || path.contains('/similar')) {
      return json({'results': []});
    }
...
class _FixtureApi extends SeerrApi {
  ...
  SeerrService build() {
    final account =
        ref.watch(userProvider.select((value) => value?.seerrCredentials));
    var active = true;
    ...
    final client = ChopperClient(
        ...
        interceptors: [
          SeerrRequest(
              jmsSeerrSource,
              {
                if (account?.sessionCookie.isNotEmpty == true)
              ...
              expectedJellyfinUserId: ref.read(userProvider)?.id,
