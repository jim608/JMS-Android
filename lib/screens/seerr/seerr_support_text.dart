...
String seerrError(BuildContext context, Object error) {
  ...
  final messages = <String, (String, String)>{
    'request_rejected_unknown': ('Request denied; cause not yet confirmed. Copy the connection diagnostic.',
        '請求被拒絕，原因待確認。請複製連線診斷。'),
    'unexpected_html': ('The API returned a non-JSON page. Check the connection diagnostic.',
        'API 回傳非 JSON 頁面，請檢查連線診斷。'),
    'access_challenge': ('An access challenge blocked the API. Password retry will not help.',
        '存取驗證阻擋 API，重試密碼無法解決。'),
    'edge_dns_error': ('Cloudflare Error 1000 blocked the API before Seerr login. Check this domain\'s DNS routing.',
        'Cloudflare Error 1000 在 Seerr 登入前阻擋 API，請檢查此網域的 DNS 路由。'),
    'edge_rejected_unknown': ('The edge service rejected the API request before Seerr login.',
        '邊緣服務在 Seerr 登入前拒絕 API 請求。'),
    'login_policy_denied': ('Seerr rejected this login under its account policy.',
        'Seerr 帳號政策拒絕此次登入。'),
    'connection_timeout': ('Connection timed out. Retry when the service is reachable.',
        '連線逾時，服務恢復後請重試。'),
    'dns_error': ('The service hostname could not be resolved.', '無法解析服務主機名稱。'),
    'tls_error': ('Secure connection failed. Check the certificate and service URL.',
        '安全連線失敗，請檢查憑證與服務網址。'),
    'service_access_blocked': ('Service/proxy access was refused before account verification. No password retry.', '服務或反向代理在帳號驗證前拒絕連線；不會重試密碼。'),
    'binding_unverified': ('Cannot securely verify the advertised Jellyfin server.', '無法安全核對 Seerr 公告的 Jellyfin 伺服器。'),
    'identity_mismatch': ('Identity mismatch. No session was adopted.', '身分或伺服器不相符，未採用此工作階段。'),
    'binding_required': ('Confirm the service binding first.', '請先確認此 Jellyfin 與點片服務的來源綁定。'),
    'needs_auth': ('One-time verification needed; library login is preserved.', '需要一次重新驗證；媒體庫登入仍保留。'),
    'link_unsupported': ('Server is not initialized for supported Jellyfin login.', '服務尚未完成可支援的 Jellyfin 登入設定。'),
    'service_unavailable': ('Request service or secure storage unavailable; playback is unaffected.', '點片服務或安全儲存無法使用；不影響播放。'),
